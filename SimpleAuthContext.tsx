...
import { TelegramAuthService, TelegramAuthResponse, TelegramWidgetAuthData } from '../services/TelegramAuthService';
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const logout = async () => {
    ...
    // Increment webview_reset_key so TelegramLoginScreen fully remounts its
    // WebView on the next login attempt, clearing any cached Telegram session.
    try {
      const prev = await AsyncStorage.getItem('webview_reset_key');
      await AsyncStorage.setItem('webview_reset_key', String(parseInt(prev || '0') + 1));
    } catch (_) {
      // Non-fatal — clear() below handles full cleanup
    }
  ...
  /**
   * WebView HMAC flow (new).
   * Receives the raw Telegram Login Widget payload from TelegramLoginScreen,
   * verifies it server-side via the Edge Function, and establishes a session.
   *
   * Security steps (mirror the guide):
   *   1. Always sign out first — prevents session bleed between accounts.
   *   2. Call the telegram-auth Edge Function for HMAC verification.
   *   3. Verify the returned session belongs to the authenticating Telegram user.
   *   4. Set the session and load the user profile.
   *   5. On any error, sign out to never leave a partial session active.
   */
