import React, { useEffect, useImperativeHandle, forwardRef, useMemo, useCallback, useState } from 'react'
import {
  View,
  StyleSheet,
  Modal,
...
const BottomSheet = forwardRef<BottomSheetRef, BottomSheetProps>(({
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      const isAtFullScreen = currentSnapIndex.value === snapPoints.length - 1 && snapPoints[snapPoints.length - 1] >= 1.0
    ...
    .onEnd((event) => {
      ...
      const targetY = targetPoint >= 1.0
        ? -SCREEN_HEIGHT
        : -(SCREEN_HEIGHT * targetPoint)
      translateY.value = withSpring(targetY, {
        damping: 25,
        stiffness: 300,
        mass: 0.8,
