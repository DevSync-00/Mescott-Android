...
export class BookingService {
  ...
  static async updateBookingAndTaskStatus(bookingId: string, status: Booking['status'], updatedByUserId?: string): Promise<boolean> {
    try {
      ...
      if (bookingId.startsWith('task_')) {
        ...
        // If task is completed, run background operations without blocking
        if (status === 'completed') {
          // Don't wait for these operations - they run in background
          this.handleCompletedTaskBackground(application.task_id, application.tasks).catch(error => {
            console.error('Background completion operations error (non-critical):', error)
          })
          
          // Send notifications in background too
          this.sendBookingStatusNotifications(application.tasks, status, application.task_id).catch(error => {
            console.error('Background notification error (non-critical):', error)
          })
          
          // Return success immediately without waiting for background operations
          return true
        }

        // Send notifications for non-completed status updates (runs synchronously)
  ...
  // Background operations for completed tasks (doesn't block UI)
  private static async handleCompletedTaskBackground(taskId: string, task: any): Promise<void> {
    try {
      console.log('🔄 Running background operations for completed booking task:', taskId)
      ...
      if (taskDetails) {
        ...
        // Create payment requirement (non-blocking)
        PaymentService.createTaskPayment(
          ...
          console.error('Error creating payment requirement (non-critical):', error)
        })

        // Notify tasker that payment is ready (non-blocking)
        if (taskDetails.tasker_id) {
          const { UnifiedNotificationService } = await import('./UnifiedNotificationService')
          UnifiedNotificationService.notifyTaskerPaymentReady(
            taskId,
            taskDetails.title,
            taskDetails.tasker_id,
            paymentAmount
          ).catch(error => {
            console.error('Error notifying tasker (non-critical):', error)
          })
        }
      }

      // Delete chat and all messages for this task (non-blocking)
      try {
        console.log('🗑️ Deleting chat for completed booking task:', taskId)
        ...
        console.error('Error deleting chat for completed booking task (non-critical):', chatError)
      }

      console.log('✅ Background operations completed for booking task:', taskId)
    } catch (error) {
      console.error('Error in background operations (non-critical):', error)
