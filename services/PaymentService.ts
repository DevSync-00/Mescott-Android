...
export type { PaymentCalculation } from './ChapaPaymentService'
...
export class PaymentService {
  ...
  static async initializeChapaPayment(
    ...
    try {
      ...
      const isSuccessResponse = (payload: any): payload is { checkoutUrl: string; txRef: string } =>
        payload && typeof payload.checkoutUrl === 'string' && typeof payload.txRef === 'string'

      if (!result || !isSuccessResponse(result)) {
        const errorMessage = (result as any)?.error || 'Failed to initialize payment'
